...
class GMMMissing(BaseEstimator, ClusterMixin):
    ...
    def __init__(
        ...
        if init_params not in ("kmeans", "random"):
            raise ValueError(f"init_params must be 'kmeans' or 'random', got {init_params}")
    ...
    def _e_step(
        ...
        for k in range(self.n_components):
            try:
                ...
                pseudo_cov = self.covariances_[k] + np.eye(X.shape[1]) * (self.reg_covar * 10)
                rv = multivariate_normal(
                    mean=self.mu_[k],
                    cov=pseudo_cov,
                    allow_singular=True
    ...
    def _m_step_params(
        ...
        for k in range(self.n_components):
            if N_k[k] < 1e-15:
                logger.warning(f"Component {k} has negligible responsibility, skipping update")
                continue
        ...
        logger.debug(f"M-step completed: updated parameters for {self.n_components} components")
    ...
    def _m_step_data(
        ...
        for k in range(self.n_components):
            try:
                ...
                reg_cov = self.covariances_[k] + np.eye(n_features) * (self.reg_covar * 10)
                precisions[k] = np.linalg.inv(reg_cov)
                logger.warning(f"Added regularization for precision matrix of component {k}")
    ...
    def fit(self, X_incomplete: NDArray[np.floating]) -> "GMMMissing":
        ...
        # Run EM iterations
        logger.info(f"Starting EM algorithm with max_iter={self.max_iter}, tol={self.tol}")
